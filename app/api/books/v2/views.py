...
class BorrowBook(Resource):
    ...
    def post(self,id):
        ...
        result = tk.decode_token(self,validate_token)
        # print(result)
        if isinstance(result,str):
            return make_response(jsonify({
                'message':'Invalid Token.Please Login'
            }),400)
        user_creds = user.get_user_id(result)
        if not user_creds:
            return make_response(jsonify({
                'message':'Please Log In'
            }),400)
        ...
        response = book.borrow_book(status,id,result)
        # print(response)
        if response:
            return make_response(jsonify({
...
#         get_books = book.get_unreturned_books(response)
#         if not get_books:
#             return make_response(jsonify({
