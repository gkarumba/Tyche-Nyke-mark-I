...
class ValidationsTest(BaseTest):
    ...
    def test_check_word(self):
        response = self.client.post('api/v1/books',data=json.dumps(self.word_data),content_type='application/type')
        result = json.loads(response.data)
        self.assertEqual(response.status_code, 400)
        self.assertIn(result['message'], 'Author takes only letters')

    def test_check_status(self):
        self.client.post('/api/v1/books',data=json.dumps(self.post_data),content_type='application/type')
        response  = self.client.put('/api/v1/books/borrow/1',data=json.dumps(self.status_data),content_type='application/json')
        result = json.loads(response.data)
        self.assertEqual(response.status_code, 400)
